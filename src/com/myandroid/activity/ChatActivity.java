...
public class ChatActivity extends Activity implements OnClickListener {
	...
	private final Runnable scrollRunnable = new Runnable() {
		@Override
		public void run() {
			int offset = chartMsgPanel.getMeasuredHeight()
					- chartMsgScroll.getHeight();// 判断高度
			if (offset > 0) {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Tools.chart = this;
		setContentView(R.layout.activity_chartv);
		...
		chartTTS = (Button) findViewById(R.id.chart_tts_button);
		chartTTS.setOnClickListener(new OnClickListener() {

			@Override
			public void onClick(View arg0) {
				int length = MyRecognizerDialogLister.words.length();
				MyRecognizerDialogLister.words.delete(0, length);
				VoiceToWord voice = new VoiceToWord(ChatActivity.this,
						"534e3fe2");
	...
	public void sendMediaRecord(String name) {
		...
		ImageView senduserhead = (ImageView) view
				.findViewById(R.id.senduserhead);
		senduserhead.setImageResource(Tools.headIconIds[Tools.me
				.getHeadIconPos()]);
		MediaPlayer mp = MediaPlayer.create(ChatActivity.this,
				Uri.parse(media.sendpath + "/" + name));
		...
		String body = (new File(add)).getName() + Tools.sign
				+ (new File(add)).length();
		Tools.out("body:" + body);
		Msg msg = new Msg(Tools.me.getHeadIconPos(), Tools.me.getName(),
				Tools.me.getIp(), person.getName(), person.getIp(),
				Tools.CMD_SENDMEDIA, body);
		tools.sendMsg(msg);
		// 更新滚动条
	...
	private void receiveMsg(Msg m) {
		View view = getLayoutInflater().inflate(R.layout.received_msg_layout,
				null);
		TextView smcView = (TextView) view
				.findViewById(R.id.received_msg_content);
		...
		ImageView receiveIcon = (ImageView) view
				.findViewById(R.id.receiveuserhead);
	...
	private void receiveMedia(Msg m) {
		...
		MediaPlayer mp = MediaPlayer.create(ChatActivity.this,
				Uri.parse(media.receivepath + "/" + fileInfo[0]));
		int duration = mp.getDuration() / 1000;// 即为时长 是ms
		// 得到录音时间
		smcView.setText(duration + "″((");
	...
	private void receiveCall(Msg msg) {
		if (!Tools.stoptalk) {
			...
			Button talkOkBtn = (Button) vi
					.findViewById(R.id.receive_talk_okbtn);
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case RESULT_OK:
			...
			Msg msg = new Msg(Tools.me.getHeadIconPos(), Tools.me.getName(),
					Tools.me.getIp(), person.getName(), person.getIp(),
					Tools.CMD_FILEREQUEST, (new File(choosePath)).getName()
							+ Tools.sign + (new File(choosePath)).length());
			tools.sendMsg(msg);
			Tools.out("发送请求传送文件");
		}
	}
