...
public class Tools {
	...
	public static int[] headIconIds = { R.drawable.face0, R.drawable.face1,
			...
			R.drawable.face14, R.drawable.face15 };
	...
	public void parse(Msg msg) {
		...
		case Tools.CMD_FILEACCEPT:
			...
			tc0.start();
			Tools.TipsChat(Tools.FILE_JINDU,
					"发送文件" + Tools.sign + "正在发送：" + new File(path).getName()
		...
		case Tools.CMD_SENDMEDIA:// 对方发送录音文件
			...
			ts.start();
			// 发送消息 让对方开始发送文件
			Msg msg0 = new Msg(0, msg.getReceiveUser(), msg.getReceiveUserIp(),
					msg.getSendUser(), msg.getSendUserIp(),
					Tools.CMD_PREPAREFILE, null);
			sendMsg(msg0);
			break;
	...
	public void receiveMsg(Msg msg) {
		...
		if (Tools.State == Tools.MAINACTIVITY) {
			...
			// 存储缓存消息，并提示条数
			List<Msg> mes = null;
			if (msgContainer.containsKey(msg.getSendUserIp())) {// 如果存在此人的消息缓存
	...
	public boolean judgeUser(Msg msg) {// false 表示不存在
		for (int i = 0; i < mainA.userList.size(); i++) {
			if (mainA.userList.get(i).getIp().equals(msg.getSendUserIp())) {
				...
				if (mainA.userList.get(i).getHeadIconPos() != msg
						.getHeadIconPos()) {
					mainA.userList.get(i).setHeadIconPos(msg.getHeadIconPos());// 该在线列表的名字
					mainA.adapterList.get(i).put("headicon",
							Tools.headIconIds[msg.getHeadIconPos()]);
					// 刷新列表
					Tips(Tools.FLUSH, null);
				}
				return true;
			...
			while (!mainA.isPaused) {
				for (int i = 0; i < mainA.userList.size(); i++) {
					long cm = System.currentTimeMillis()
							- mainA.userList.get(i).getOnlineTime();
